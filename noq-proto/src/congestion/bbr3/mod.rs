...
mod test {
    ...
    /// has data and is paced at BBR's chosen rate, so it is cwnd-limited (never
    /// application-limited). This is the shared harness behind the constant-link
    /// tests (A.1/A.3/A.5/A.8/A.9/A.10); tests that need loss, application
    /// limiting, a mid-flight bandwidth change, idle periods, or ACK aggregation
    /// drive their own bespoke loops instead.
    struct Sim {
        bbr: Bbr3,
        base: Instant,
    ...
    fn startup_exits_to_drain_on_loss_when_app_limited() {
        ...
        // app-limited, so it was app-limiting -- not a race with loss -- that kept
        // the plateau path from completing. The only packets sent non-app-limited are
    ...
    ///
    /// Same infinite-buffer, single-bottleneck FIFO simulator as A.1/A.8 (constant
    /// link rate, constant propagation delay), driven STARTUP -> DRAIN -> PROBE_BW
    /// and then left to run for more than `BBR.ProbeRTTInterval` (5 s). This is the
    /// periodic min-RTT re-probe: once the recorded `BBR.probe_rtt_min_delay` has not
    /// been lowered for a full ProbeRTTInterval, `update_min_rtt` flips
    ...
    /// rate and constant propagation delay, the smallest achievable RTT is the fixed
    /// floor `FWD + service + RET`, hit whenever the bottleneck queue is empty (every
